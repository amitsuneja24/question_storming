    ...
    return Scaffold(
      ...
      body: StreamBuilder(
        ...
        builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
          List questionList = snapshot
              .data.documents[baseindex].data['Questions']?.keys
              ?.toList();
            ...
            children: <Widget>[
              ...
              Expanded(
                child: ListView.builder(
                    itemCount: answerList[topIndex].length,
                    itemBuilder: (context, int index) {
                      return (noOfAnswers != 0)
                          ? Container(
                              height: 30,
                              width: double.infinity,
                              child: Center(
                                child: Text(
                                    '${answerList[topIndex][index].values.toList()[0]}'),
                              ),
                            )
                          : Center(
                              child: Text(
                ...
                onPressed: () async {
                  if ((answerController.text != null) &&
                    ...
                    await firebasedb.document(name).setData({
                      'Questions': {
                        '${questionList[topIndex]}': FieldValue.arrayUnion([
                          {'${nameController.text}': '${answerController.text}'}
                        ])
                      }
                    }, merge: true);
